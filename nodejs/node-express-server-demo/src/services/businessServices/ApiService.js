...
exports.getnewslist = (req, res) => {
   ...
   */
  var sql = " SELECT id,title,add_time,left(zhaiyao,25) as zhaiyao,click,concat('" + domain + "',img_url) as img_url FROM dt_article where img_url > '' and channel_id = 6 limit 0,10 "
...
exports.getgoods = (req, res) => {
  ...
  ,img_url:图片地址，前缀是7牛云存储域名
   */
  var sql = `SELECT a.id,a.title,a.add_time,left(a.zhaiyao,25) as zhaiyao,a.click,concat('${domain}',a.img_url) as img_url,b.sell_price,b.market_price,b.stock_quantity FROM dt_article as a,dt_article_attribute_value b where a.id = b.article_id and a.channel_id = 7 limit ${skipcount},${pagesize} `
  console.log('获取图文资讯sql语句：============>', sql)
  req.db.driver.execQuery(sql, (err, datas) => {
    // 4.0 判断是否异常
    if (err) {
...
exports.getshopcarlist = (req, res) => {
  ...
  // 2.0 执行查询操作
  var sql = `SELECT count(distinct tb1.id) as cou, tb1.* FROM (
...
exports.getimages = (req, res) => {
  ...
  var sql = ' select id,title,img_url,zhaiyao from dt_article where channel_id = 9 and category_id=' + cateid
  if (cateid <= 0) {
...
exports.getimage = (req, res) => {
  ...
  // 2.0 执行查询操作
  var sql = `select thumb_path as src  from dt_article_albums where article_id =${newid}`
...
exports.getimgcategory = (req, res) => {
  ...
  var resObj = { status: successState, message: '' }
  // 3.0 利用orm发送sql语句查询出来分页数据即可
  var sql = ' select title,id from dtcmsdb4.dt_article_category where channel_id = 9 '
...
exports.postcomment = (req, res) => {
  ...
  //获取评论内容
  req.on('data', (chunk) => {
    var commentTxt = chunk.toString();
    const qs = require('querystring');
    var commentObj = qs.parse(commentTxt);
    // 2.0 执行查询操作
